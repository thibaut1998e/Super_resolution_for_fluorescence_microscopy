...
def wnres_unet_model(arch, in_c=1, out_c=1, blur=True, blur_final=True, self_attention=True, last_cross=True, bottle=True, norm_type=NormType.Weight, **wnres_args):
...
def wnres_unet_learner(data, wnres_args,**kwargs):

    scale = wnres_args.pop('scale')
    print('wnresnet' ,wnres_args)

    model, meta = wnres_unet_model(**wnres_args)
    ...
    learn.model = BilinearWrapper(learn.model, scale=scale)
