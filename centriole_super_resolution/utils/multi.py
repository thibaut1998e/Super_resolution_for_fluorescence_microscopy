...
class MultiImage(ItemBase):
    #a list of tensors  each of them represents a 3d image
    ...
    def data(self):
        ...
        #the fuction view create a new tensor that shares the same data but with different shape
    ...
    def apply_tfms(self, tfms, **kwargs):
        ...
        for img in self.img_list:
            #do resolve : if False, the values of random parameters are kept from the last draw
...
class MultiImageList(ImageList):
    ...
    def open(self, fn):
        img_data = np.load(fn, allow_pickle=True)
